...
export default function HeadeTop(props) {
    ...
    return (
        ...
        <View  style={navbar}>
            ...
            <View style={{ flexDirection : 'row', alignItems : 'center' }}>
                <IconButton onPress={() => navigation.navigate('Login') }  icon={{ uri :'https://cdn-icons-png.flaticon.com/512/450/450387.png' }} size={21} color="#fff"></IconButton>
                {!props.hasUser &&(
                    <Text style={{ color : '#fff'}}> <Text style={{ fontSize : 15 }}>Welcome</Text> <Text style={{ fontSize : 19, fontWeight : '600', textTransform : 'capitalize' }}> {curentUser?.user?.username}</Text></Text>
                ) }
            </View>
        </View>
    )
